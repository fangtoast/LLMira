...
function formatSaveError(error: unknown): string {
  if (error instanceof Error) return `保存失败：${error.message}`;
  return "保存失败：设备凭据库没有接受这次写入，请稍后重试。";
}
...
export function ProviderSetupForm({ compact = false, showProfileList = true, onComplete }: { compact?: boolean; showProfileList?: boolean; onComplete?: () => void }) {
  ...
  const [errorKind, setErrorKind] = useState<"scan" | "save">("scan");
  ...
  useEffect(() => {
    let cancelled = false;
    void useSettingsStore.getState().hydrateProviderSecrets().then(() => {
      if (cancelled) return;
      const hydrated = useSettingsStore.getState().apiProfiles.find((profile) => profile.id === active.id);
      if (hydrated?.apiKey) setApiKey((current) => current || hydrated.apiKey);
    }).catch((caught: unknown) => {
      if (cancelled) return;
      logger.exception(caught, "provider secret hydration failed", { providerId: active.id });
    });
  ...
  async function scan() {
    setScanning(true); setError(undefined); setErrorKind("scan"); setSaved(false);
    try {
  ...
  async function save() {
    ...
    if (!trimmedApiKey) { setError("请先填写 API Key，再保存 Provider。"); setErrorKind("save"); return; }
    setSaving(true); setError(undefined); setErrorKind("save"); setSaved(false);
    try {
      await store.saveActiveApiKey(trimmedApiKey);
      ...
      logger.exception(caught, "provider secret save failed", { providerId: active.id, runtime: isTauriRuntime() ? "tauri" : "web" });
      setError(formatSaveError(caught));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className={compact || !showProfileList ? "grid gap-5" : "grid gap-7 lg:grid-cols-[240px_minmax(0,1fr)]"}>
      ...
      <div className="min-w-0 space-y-6">
        ...
        {error ? <Alert variant="destructive"><KeyRound className="size-4" /><AlertTitle>{errorKind === "save" ? "Provider 保存失败" : "Provider 连接失败"}</AlertTitle><AlertDescription>{error}</AlertDescription></Alert> : null}
        ...
        <div className="flex flex-wrap items-center gap-3"><Button type="button" onClick={() => void save()} disabled={!result || saving}>{saving ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Check className="mr-2 size-4" />}{saving ? "正在保存…" : "明确保存 Provider"}</Button>{saved ? <span className="text-sm text-emerald-600">已安全保存，可以开始对话。</span> : null}</div>
      </div>
    </div>
  );
}
