    ...
    <div className="grid gap-6">
      ...
      <SettingsCard title="搜索服务配置">
        <FieldGroup className="gap-4">
          ...
          {store.searchProvider !== "searxng" ? <Field><FieldLabel htmlFor="search-key">搜索 API Key</FieldLabel><Input id="search-key" type="password" autoComplete="off" value={key} onChange={(event) => { setKey(event.target.value); setSaved(false); }} placeholder="请输入 API Key" /><FieldDescription>原生端保存在 Stronghold，Web 仅保留当前会话。</FieldDescription></Field> : null}
        </FieldGroup>
