...
fn wire_name(server_id: &str, tool_name: &str) -> String {
    ...
    let full = format!(
        "mcp_{}_{}",
        slug,
        fnv1a(&format!("{}\0{}", server_id, tool_name))
    );
...
fn config_fingerprint(input: &McpConnectionInput) -> String {
    format!(
        "{}:{}:{}:{}",
        input.config.id, input.config.updated_at, input.config.transport, input.config.url
    )
}
...
async fn open_client(
    ...
    if input.config.transport == "streamable_http" {
        if input.config.url.trim().is_empty() {
            return Err("请填写 MCP 服务 URL。".into());
        }
        ...
        for entry in &input.config.headers {
            ...
            if let Some(value) = value {
                let name = HeaderName::try_from(entry.name.as_str())
                    .map_err(|_| format!("无效请求头：{}", entry.name))?;
                let value = HeaderValue::try_from(value)
                    .map_err(|_| format!("请求头值无效：{}", entry.name))?;
                headers.insert(name, value);
            }
        }
        let mut config = StreamableHttpClientTransportConfig::with_uri(input.config.url.clone())
    ...
    #[cfg(not(mobile))]
    {
        if input.config.command.trim().is_empty() {
            return Err("请填写 STDIO 启动命令。".into());
        }
        let mut command = tokio::process::Command::new(&input.config.command);
        command.args(&input.config.args);
        if !input.config.cwd.trim().is_empty() {
            command.current_dir(&input.config.cwd);
        }
        for (name, value) in input.environment.as_ref().into_iter().flatten() {
...
pub async fn mcp_list_tools(
    ...
    if snapshot.status == "connected" {
        ...
        Err(snapshot.error.unwrap_or_else(|| "MCP 连接失败".into()))
    }
}
...
pub async fn mcp_call_tool(
    ...
    if snapshot.status != "connected" {
        return Err(snapshot.error.unwrap_or_else(|| "MCP 连接失败".into()));
    }
    let session = state
        .sessions
        .lock()
        ...
        .get(&input.config.id)
        .cloned()
        .ok_or_else(|| "MCP 会话不可用".to_string())?;
    ...
    let summary = serde_json::to_string(&result.structured_content.as_ref().unwrap_or(&content))
        .unwrap_or_else(|_| "工具已完成".into());
    Ok(ToolCallResponse {
        content,
...
#[tauri::command]
pub async fn mcp_cancel_call(
    state: tauri::State<'_, McpRuntimeState>,
    call_id: String,
) -> Result<(), String> {
    if let Some(sender) = state.cancellations.lock().await.remove(&call_id) {
        let _ = sender.send(());
    }
    Ok(())
}
